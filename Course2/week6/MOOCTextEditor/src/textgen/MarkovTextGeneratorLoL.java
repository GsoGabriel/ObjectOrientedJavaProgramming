...
public class MarkovTextGeneratorLoL implements MarkovTextGenerator {
	...
	public void train(String sourceText)
	{
		...
		String[] srcTextSplit = sourceText.split(" +");
		starter = srcTextSplit[0];
		String prevWord = starter;
		for (int i = 1; i < srcTextSplit.length; i++) {
			String currWord = srcTextSplit[i];
			ListNode prevNode = new ListNode(prevWord);
			int index = wordList.indexOf(prevNode);
			if (index != -1){
				wordList.get(index).addNextWord(currWord);
			}
			else{
				wordList.add(prevNode);
				index = wordList.indexOf(prevNode);
				wordList.get(index).addNextWord(currWord);
			}
			if(i == srcTextSplit.length - 1){
				prevNode = new ListNode(currWord);
				index = wordList.indexOf(prevNode);
				if (index != -1){
	...
	public String generateText(int numWords) {
		 ...
		 * 	add "w" to the "output"   set "currWord" to be "w"
		 * 	increment number of words added to the list
		 */
		if (starter == null || starter.isEmpty() || numWords == 0) {
			return "";
		}
		...
		while (i < numWords){
			...
			int index = wordList.indexOf(currNode);
...
class ListNode
{
	...
	public boolean equals(Object obj){
		...
		if (this.getWord().equals(node.getWord())){
			return true;
		}
		else{
			return false;
